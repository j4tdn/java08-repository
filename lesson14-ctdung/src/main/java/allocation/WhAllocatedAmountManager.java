...
public class WhAllocatedAmountManager {
	...
	private static Map<Long, Integer> doAllocation(List<Store> data, Integer allocationAmount) {
		//1.
		BigDecimal avg =  data.stream().map(Store::getExpectedSales).filter(Objects::nonNull).reduce(BigDecimal.ZERO, BigDecimal::add);
		
		
		
		Map<Long, Integer> map1 = data.stream()
				.filter(s -> s.getSelected())
				.map(Store::getExpectedSales)
				.filter(Objects::nonNull).collect(Collectors.toMap(Store::getStoreId, Store::getExpectedSales));
