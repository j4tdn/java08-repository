...
public class BookUtils {
	public static void findPublisher(Book[] books) {
		for (Book book : books) {
			if(book.getPublisher().equals("Nhi dong")) System.out.println(book.getId());
		}
		
	}
	public static void findBook(Book[] books) {
		for (Book book : books) {
			if(book.getPrice() < 50) System.out.println(book.getId());
		}
	}
	public static void findReferenceBook(Book[] books) {
		for (Book book : books) {
			if(book.getId().startsWith("SGK") && book.getPrice() >= 100 && book.getPrice() <= 200) System.out.println(book.getId());
		}
